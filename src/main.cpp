 ...
 *      tourner à droite => angle négatif
 *      tourner à gauche => angle positif
 * 
 * Pour les TOF la numérotation est 
 *    0 à droite
 *    1 à gauche
 *    2 vers le bas (ce TOF peut être exclu de la boucle de lecture - cas des FAN)
 * 
 * Paramètres du robot
 * ===================
 * Certains paramètres (#define) ont migré vers pami.h
 * Cela permet de faire des #include "pami.h" là où nécessaire
 * 
 * Mouvements et odométrie
 * =======================
 * Cette version introduit l'odométrie, c'est à dire la connaisance de la position et de l'orientation 
 * du robot (non pas en continu mais à la fin de chaque mouvement unitaire).
 * 
 * Les mouvements sont des instances d'une classe voir mouvement.h et mouvement.cpp
 * Principe d'utililsation 
 *    instancier un objet mouvement 
 *    appeler la méthode go qui initialise le mouvement
 *    appeler la méthode de paramétrage de la détection d'obstacle du mouvement (non implémentées au 23/02)
 *        ou appeler initDetection pour chaque TOF
 *    le mouvement sera exécuté par la boucle loop qui doit inclure 
 *        les run des steppers et 
 *        la détection d'obstacle qui 
 *            lève une alarme (variable globale arretSurAlarme) et 
 *            fait des stepper.stop()
 *    quand les steppers sont arrétés (fin de mouvement ou arrêt sur alarme) 
 *        on fait un update de la position du robot via la méthode updatePosition 
 *        du mouvement qui vient de se terminer.
 *        Cet update tient compte d'un arrêt sur alarme via la variable globale arretSurAlarme
 * 
 * La position du robot est définie par 
 *    coordonnées x et y en mm
 *    orientation du robot angle teta avec l'axe x (sens trigonométrique)
 * Voir position.h et position.cpp
 * La position (et orientation) initiale est précisé à l'instanciation de l'objet positionCourante ou dans le setup.
 * Pour éviter des angles hors des valeurs -180 +180, utiliser la fonction setTeta qui normalera l'angle.
 * 
 * Détection d'obstacle 
 * ====================
 * Les TOF fonctionnent en continu
 * Ils sont lus à un intervalle d'environ 40ms (TIME_BETWEEN_MEASUREMENT)
 * 2 lectures de TOF différents sont faites avec un intervalle minimum de 5 ms (TIME_BETWEEN_TOF)
 * Le bus I2C est utilisé pour ces lectures à 600kHz soit plus rapide que les spécifications
 * cela permet d'avoir un temps de lecture d'un TOF inférieur à 500µs
 * 
 * Ainsi les lectures des TOF ne devraient pas trop perturber la génération des pulses moteur
 * 
 * Le test de dépassement de seuil est fait à chaque lecture effective de TOF (acquisition d'une nouvelle valeur)
 * Il n'y a pas de filtrage de vraissemblance pour les mesures => possiblité de fausse alarme 
 *    lumière environnante trop vive
 *    sol entrant dans le FOV (25°) de l'optique du détecteur.
 * paramètre défini par initDetection (actif ?, seuil ?, sens < ou >)
 * 
 * Dans une prochaine version, cette partie sera placée dans une source séparé pour une meilleure modularité
 * et on reverra les interactions avec les fonctions de mouvements et de séquence de mouvements.
 * 
 * Enchaînement de mouvements
 * ==========================
 * La fonction nextTarget sert dans cette version à introduire une séquence de mouvements à exécuter.
 * Cette séquence est linéaire et statique.
 * Le robot s'arrête définitivement en cas de détection d'obstacle.
 * La dernière étape de la séquence est le desable des drivers moteurs.
 * 
 */
...
positionCourante.setTeta(-135.0);
delay(5000U); // attente pour faciliter prise de video
}
...
#define TARGET_END_RUN 9
#define TARGET_ARRET_MOTEUR 8
/** Détermine le prochain mouvement 
* Peut Comporter des temps d'attente
*/
void nextTarget(){
    static uint16_t numTarget = 0; // numéro du target en cours
    static bool alarmTraite = false; 
    if (arretSurAlarme && ! alarmTraite){
      afficheLigne(50,14,"Arret sur alarme");
      numTarget = TARGET_ARRET_MOTEUR ;
      alarmTraite = true;
    } else {
      if (numTarget < TARGET_END_RUN ) numTarget++;
    }
    switch (numTarget) {
      case 1 :
/*
        ...
        afficheLigne(16,16,"tout droit 100 mm");
        mvtEnCours = new MvtToutDroit(100);
        mvtEnCours->go();
      ...
      case 2 :
        ...
        afficheLigne(16,16,"rotation 90° à droite");
        mvtEnCours = new Rotation(-90.0); // sens trigo
        mvtEnCours->go();
        initDetection(0,false,100,true); // tof à droite
        initDetection(1,false,100,true); // tof à gauche
        break;
*/
        if (traceMouvement) {
          Serial.println(positionCourante.toString().c_str());
        }
        afficheLigne(16,16,"virage r=100, d=-90° (à droite)");
        mvtEnCours = new Virage(100.0,60.0);
        mvtEnCours->go();
        initDetection(0,false,100,true); // tof à droite
        initDetection(1,false,100,true); // tof à gauche
        break;
      case TARGET_ARRET_MOTEUR :
        afficheLigne(16,16,"arret moteurs et fin");
        desableMotorDrivers();
        break;
      default :
        // fin ou étape sans action
        break;
    }
}
